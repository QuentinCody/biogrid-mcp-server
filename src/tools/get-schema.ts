...
export function registerGetSchema(server: McpServer, env?: SchemaEnv): void {
	const handler = createGetSchemaHandler("BIOGRID_DATA_DO", "biogrid");

	server.registerTool(
		"biogrid_get_schema",
		{
			title: "Get Staged BioGRID Schema",
			description:
				"Get schema information for staged BioGRID data. Shows table structures and row counts. " +
